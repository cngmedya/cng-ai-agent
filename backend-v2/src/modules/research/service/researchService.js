...
async function generateFullResearch({ leadId }) {
  ...
  // 1) intel basic
  let intelBasic = null;
  try { intelBasic = await intel.analyzeLead({ leadId }); }
  catch (e) { console.error('[research] Basic intel hata:', e.message); }

  // 2) intel deep
  let intelDeep = null;
  if (lead.website) {
    try { intelDeep = await intel.analyzeLeadDeep({ leadId }); }
    catch (e) { console.error('[research] Deep intel hata:', e.message); }
  }

  // 3) web search
  let webPresence = null;
  try { webPresence = await runWebSearch(lead); }
  catch (e) { console.error('[research] Web search hata:', e.message); }

  // 4) socials
  let socialPresence = null;
  try { socialPresence = await detectSocials(lead, webPresence); }
  catch (e) { console.error('[research] Socials hata:', e.message); }

  // 5) ads
  let adIntel = null;
  try { adIntel = await analyzeAds(lead, webPresence); }
  catch (e) { console.error('[research] Ads hata:', e.message); }

  // 6) competitors
  let competitors = null;
  try { competitors = await findCompetitors(lead, webPresence); }
  catch (e) { console.error('[research] Competitors hata:', e.message); }

  // 7) benchmark
  let benchmark = null;
  try {
    benchmark = await benchmarkLead(lead, {
      intelBasic,
      intelDeep,
      webPresence,
      socialPresence,
  ...
  } catch (e) { console.error('[research] Benchmark hata:', e.message); }
  ...
  const aiResponse = await chatJson({
    system: 'You are CNG Medya için kurumsal B2B istihbarat motoru. Sadece geçerli JSON döndür.',
    user: JSON.stringify({
  ...
  if (!aiResponse.ok) {
    ...
    console.error(
      `[research/full-report] ❌ ERROR (leadId=${leadId}, duration=${duration.toFixed(2)}s):`,
      aiResponse.raw
    );
  ...
  const duration = (Date.now() - startedAt) / 1000;
  console.log(
    `[research/full-report] ✅ OK (leadId=${leadId}, duration=${duration.toFixed(2)}s, score=${overallScore ?? 'n/a'})`
  );
